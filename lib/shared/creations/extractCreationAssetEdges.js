  ...
  const openingLocation =
    data.opening_location && typeof data.opening_location === "object"
      ...
      : data.openingLocation && typeof data.openingLocation === "object"
        ...
        : {};
  const openingMode = normalizeType(openingLocation.mode);

  if (openingMode === "PLAYER_SELECT") {
    const allowedLocationReferences = getReferenceArray(openingLocation, [
      "allowedLocations",
      "allowed_locations",
    ]);
    const allowedLocationTargets = allowedLocationReferences.length
      ? allowedLocationReferences
      : getReferenceArray(openingLocation, [
          "allowedLocationIds",
          "allowed_location_ids",
        ]);

    allowedLocationTargets.forEach((location, index) => {
      pushEdge(edges, {
        target: location,
        relationType: "USES_LOCATION",
        role: "LOCATION",
        sortOrder: 120 + index,
        field: "opening_location.allowedLocationIds",
        sourceType,
        metadata: {
          opening_location_mode: "PLAYER_SELECT",
        },
      });
    });
  } else {
    pushEdge(edges, {
      target:
        getReferenceObject(openingLocation, [
          "fixedLocation",
          "fixed_location",
          "fixedLocationId",
          "fixed_location_id",
        ]) || getReferenceObject(data, ["selected_location", "location_id"]),
      relationType: "USES_LOCATION",
      role: "LOCATION",
      sortOrder: 120,
      field: openingMode ? "opening_location.fixedLocationId" : "selected_location",
      sourceType,
      metadata: openingMode
        ? {
            opening_location_mode: "FIXED",
          }
        : {},
    });
  }
