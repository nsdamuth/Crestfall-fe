...
export function getFirstAssignedCreationImageUrl(
  ...
  const firstAssignedMediaWithImage = featuredMedia?.find(
    (media) =>
      !media?.isPlaceholder &&
  ...
  return (
    firstAssignedMediaWithImage?.displayImageUrl ||
    firstAssignedMediaWithImage?.displayUrl ||
    firstAssignedMediaWithImage?.imageUrl ||
    firstAssignedMediaWithImage?.url ||
    fallback
  );
}
