...
function normalizeFeaturedImage(creation = {}) {
  const media = Array.isArray(creation.featuredMedia) ? creation.featuredMedia : [];
  const first = media.find((entry) =>
    normalizeText(entry?.imageUrl || entry?.url || entry?.src)
  );

  return normalizeText(
    creation.imageUrl || creation.image_url || first?.imageUrl || first?.url || first?.src
  ) || null;
}
