...
import { buildCreationCardMetrics } from "./creationCardMetrics.js";
...
function normalizeFeaturedMedia(creation = {}) {
  ...
  return media
    .map((entry, index) => {
      const displaySrc = normalizeText(getCreationMediaDisplayUrl(entry));
      if (!displaySrc) return null;

      const cardSrc = normalizeText(getCreationMediaCardUrl(entry, displaySrc));
      const thumbnailSrc = normalizeText(
        getCreationMediaThumbnailUrl(entry, cardSrc || displaySrc)
      );
      ...
      return {
        ...
        src: displaySrc,
        displaySrc,
        cardSrc: cardSrc || displaySrc,
        thumbnailSrc: thumbnailSrc || cardSrc || displaySrc,
        lockedPreviewSrc:
          normalizeText(getCreationMediaLockedPreviewUrl(entry)) || null,
        stockMediaId: entry?.stockMediaId || entry?.stock_media_id || null,
        isStockMedia: Boolean(entry?.isStockMedia || entry?.is_stock_media),
...
export function projectCommunityCreation(creation = {}, index = 0) {
  ...
  const usageMetrics = normalizeObject(creation.usageMetrics || creation.usage_metrics);
  ...
  return {
    ...
    metrics: buildCreationCardMetrics({
      creationType: type,
      usageMetrics,
      fallbackStats: stats,
    }),
