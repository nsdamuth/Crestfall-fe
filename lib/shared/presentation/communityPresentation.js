...
function normalizeFeaturedMedia(creation = {}) {
  ...
  return media
    .map((entry, index) => {
      const src = normalizeText(entry?.imageUrl || entry?.url || entry?.src);
      if (!src) return null;
...
export function projectCommunityCreation(creation = {}, index = 0) {
  ...
  const imageSrc = normalizeText(creation.imageUrl || media[0]?.src) || null;
  ...
  return {
    ...
    imageSrc,
    extraMedia: media.slice(1).map((entry) => entry.src),
