...
} from "../creations/creationMedia.js";
import {
...
// Retained as a legacy asset reference for compatibility/inventory only.
// It is no longer an automatic Story/Continue fallback.
...
function isSyntheticStoryFallbackImageSrc(value) {
  const normalized = normalizeText(value);
  return Boolean(
    normalized &&
      (normalized === STORY_CONTINUE_DEFAULT_HERO_IMAGE_SRC ||
        isLegacyDefaultCreationImageSrc(normalized))
  );
}
...
function getFirstAssignedStoryHeroMedia(media = []) {
  for (const item of Array.isArray(media) ? media : []) {
    ...
    const isExplicitStockMedia = Boolean(
      item?.isStockMedia || item?.is_stock_media || item?.stockMediaId || item?.stock_media_id
    );
    if (imageUrl && (isExplicitStockMedia || !isSyntheticStoryFallbackImageSrc(imageUrl))) {
      return typeof item === "string" ? { imageUrl } : item;
    }
  }
...
export function resolveStoryContinueImageSrc(item = {}, sourceCreation = null) {
  const assignedSourceImage = getStoryHeroMediaUrl(
    getAssignedSourceCreationMedia(sourceCreation)
  );
  if (assignedSourceImage && !isSyntheticStoryFallbackImageSrc(assignedSourceImage)) {
    return assignedSourceImage;
  }

  const roomImage = normalizeText(item?.imageSrc);
  if (roomImage && !isSyntheticStoryFallbackImageSrc(roomImage)) {
    return roomImage;
  }

  // No fake stock art: card callers receive null and render their semantic
  // full-bleed Story identity; Home renders its own gray CRESTFALL banner.
  return null;
}
