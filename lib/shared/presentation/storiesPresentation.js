...
function getAssignedSourceCreationImage(sourceCreation = {}) {
  ...
  if (Array.isArray(creation.featuredMedia) && creation.featuredMedia.length) {
    const assignedFeaturedMedia = creation.featuredMedia.filter(
      (media) => !media?.isPlaceholder
    );
    const assignedImage = getFirstCreationImageUrl(assignedFeaturedMedia, null);
  ...
  return getFirstCreationImageUrl(featuredMedia, null);
}
