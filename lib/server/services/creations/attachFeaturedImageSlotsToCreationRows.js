import {
  ...
  getImageOutputOriginalUrl,
  getImageOutputThumbnailUrl,
} from "@/lib/server/image-generation/imageOutputUrls";
...
function getImageOutputPresentationMetadata(output = {}) {
  const providerMetadata = isPlainObject(output.provider_metadata)
    ? output.provider_metadata
    : isPlainObject(output.providerMetadata)
      ? output.providerMetadata
      : {};
  const explicitPresentationMetadata = isPlainObject(output.presentation_metadata)
    ? output.presentation_metadata
    : isPlainObject(output.presentationMetadata)
      ? output.presentationMetadata
      : null;
  const crestfallPresentationMetadata = isPlainObject(
    providerMetadata.crestfallImagePresentation
  )
    ? providerMetadata.crestfallImagePresentation
    : isPlainObject(providerMetadata.crestfall_image_presentation)
      ? providerMetadata.crestfall_image_presentation
      : null;

  return explicitPresentationMetadata || crestfallPresentationMetadata || null;
}
...
function buildMediaItem({ slotKey, entry, output, title }) {
  ...
  const cardUrl = getImageOutputCardUrl(output) || displayUrl;
  const thumbnailUrl = getImageOutputThumbnailUrl(output) || cardUrl || displayUrl;
  const lockedPreviewUrl = getImageOutputLockedPreviewUrl(output);
  const originalUrl = getImageOutputOriginalUrl(output) || displayUrl;
  const presentationMetadata = getImageOutputPresentationMetadata(output);
