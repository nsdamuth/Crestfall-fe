...
export function getAncestorIds(state, folderId) {
  const ids = [];
  let current = getFolder(state, folderId);
  while (current && current.parentId) {
    ids.push(current.parentId);
    current = getFolder(state, current.parentId);
  }
  return ids;
}

// Collapse and expand (AF5 follow-up 4, item 2): a row hides while
// any ancestor is collapsed, and the chosen folder's ancestors always
// read open so the selection never hides. Each surviving row carries
// isOpen; the count stays the full subtree either way. The open state
// itself is the panel's page memory, default open.
export function applyFolderRowVisibility(state, rows, { collapsedIds = new Set(), selectedFolderId = null } = {}) {
  const keepOpen = new Set(getAncestorIds(state, selectedFolderId));
  const closed = new Set([...collapsedIds].filter((id) => !keepOpen.has(id)));
  return rows
    .filter((row) => !getAncestorIds(state, row.id).some((id) => closed.has(id)))
    .map((row) => ({ ...row, isOpen: !closed.has(row.id) }));
}
...
export const folderNotes = Object.freeze({
  ...
  // "Moved <name> into <parent>" (copy ruled at AF5 follow-up 4).
  moved: (name, targetName) => `Moved "${name}" into ${targetName === ROOT_FOLDER_LABEL ? "All" : `"${targetName}"`}.`,
