...
// referral (additive, fe/share-og follow-up 1, 13 Sep 2026, CR-073):
// the Chassis serves no referral fields on /v1/profile/me yet, so
// creditedCount reads undefined and the counter renders 0 until then.
function normalizeStudioAccountReferral(payload) {
  const referral =
    payload?.data?.referral || payload?.data?.profile?.referral || null;
  if (!referral || typeof referral !== "object") return { creditedCount: undefined };

  return {
    creditedCount: referral.creditedCount ?? referral.credited_count,
  };
}
...
function normalizeStudioAccountPayload(payload) {
  ...
  return {
    ...
    referral: normalizeStudioAccountReferral(payload),
  };
}
