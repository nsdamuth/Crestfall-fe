...
// Public creator profiles, the same list the Community and Creators
// pages load server-side through lib/server/studio/getCommunityPageData.js.
// Added for the top bar global search (FE/GLOBAL-SEARCH, 10 Sep 2026):
// the route takes no query, scope, or paging today (see
// docs/handoffs/GLOBAL-SEARCH-BACKEND.md), so the caller filters the
// list in the browser.
export async function fetchCommunityCreators() {
  const payload = await requestCreationApi({
    endpoint: "/api/community/creators",
    method: "GET",
    fallbackMessage: "Community creators could not be loaded.",
    cache: "no-store",
  });

  return payload?.data?.creators || payload?.creators || [];
}
...
export async function setCreationStockFeaturedImageSlot(
  creationId,
  slotKey,
  stockMediaId,
  fallbackMessage = "Crestfall Stock image could not be saved."
) {
  return requestCreationApi({
    endpoint: `/api/creations/${creationId}/stock-featured-image-slots/${slotKey}`,
    method: "PUT",
    body: {
      stockMediaId,
    },
