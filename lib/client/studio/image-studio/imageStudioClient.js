...
export async function createImageGenerationJob(payload) {
  const tuning = payload?.settings?.workflowTuning;
  ...
  const hasCompleteWorkflowTuning =
    tuning &&
    typeof tuning === "object" &&
    requiredTuningKeys.every((key) => Number.isFinite(Number(tuning[key])));

  if (!hasCompleteWorkflowTuning) {
    throw new Error(
      "Image settings were not attached to this generation request. Refresh Image Studio and try again."
    );
  }
