...
- **STUBBED** `/studio/* (studio shell)` / top bar (desktop) / Notifications: Opens a compact floating notification panel; idle icon with a gold-glow state when notifications exist; its Open the notification center button switches to a full panel grouped by Today/Earlier. (No notification data source exists yet; the viewmodel still defaults to an empty list. The Shell (StudioTopBar.jsx) feeds a mock list from components/studio/studio-top-bar/studioTopBarNotifications.mock.js so the bell/panels can be reviewed; delete that file plus its one import to restore the honest empty default. Dismiss and Clear all are presentation-only no-ops. Real data needs a services-api contract (CR-017, frontend complete). See docs/CONTRACT-REQUESTS.md.)
