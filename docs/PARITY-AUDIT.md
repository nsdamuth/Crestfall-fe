...
## 13. H6 catch-up, executed (branch design/h6-rows)

Section 11's eight items, disposition each:

1. DONE. `/studio/v2/community` had zero filter-bar rows in the CSV at
   all (not just a stale option list); added a Search row and the Sort
   row with the full Recommended / Most played / Newest / Most hearted
   / Most saved list and the recorded note, verified against
   `CommunityV2Mockup.jsx`'s `SORT_OPTIONS`.
2. DONE. `/studio/v2/images` Sort row (formerly row 882) updated in
   place to add Oldest, verified against `ImagesV2Mockup.jsx`.
3. DONE. Rows 862 and 1007 (creator-card "View profile") get the
   drift-closure note; no label text changed.
4. DONE. The H4 wave report does not exist on disk (confirmed: no
   file under `docs/` or in git history names it; the wave's commit,
   `ceeb801`, carries no parity echo in its message). Re-enumerated
   ten rows for `/studio/v2/adventures` directly from
   `Adventures.view.jsx` and `useAdventuresViewModel.js`: top banner,
   filter bar (search, sort), creation-card grid (like, save, expand),
   grid endcap, bottom banner, and the rehosted builder modal (which
   points at the existing rows 138 to 158 for its 27 fields rather
   than duplicating them).
5. DONE, both halves. Added two rows for the `/lore` route
   (`LoreArcAccordion`, `TimelineCard`) citing the fold ruling in their
   purpose text, and separately added twenty rows for
   `/studio/v2/lore` (also zero rows before this pass), enumerated
   from `Lore.view.jsx` and `useLoreViewModel.js`. While reading
   `LoreArcAccordion.jsx` for the row text, found the open/closed arc
   state is written to `localStorage` under one key
   (`crestfall:lore-open-arcs`, the `STORAGE_KEY` constant) but read
   back from a different literal key (`crestfall-open-arc-ids`) on
   mount, so persistence silently never restores. Recorded in the new
   row's notes; report-only per this pass's no-component-edits scope.
6. DONE. Rows 984 and 985 relabeled from block "continue strip" to
   "top banner" (the code already composes one `KitPromoBannerView`,
   conditionally showing continue content; the CSV block name had
   drifted from the merge), with the ruling 1a citation restated in
   both rows' notes.
7. DONE. Row 1002's notes field carries the state-only caveat.
8. CONFIRMED MISSING, not recovered. Searched this repo's git history
   and `docs/HANDOFF-NEXT-CHAT.md`: the Sprint D session report is not
   a file, it is the chat session's own final report (commit
   `5fb7d0c`'s message says `HANDOFF-NEXT-CHAT.md` only "points to the
   session's final chat report for the full parity echoes"). That chat
   transcript is not in this repo and cannot be recovered by this
   session. Per this brief's instruction, said so rather than
   reconstructed. Sections 2 and 8 of this audit stand as the current,
   re-run parity echo for Stories and Images; no further action taken.

Also executed, per this session's brief (outside section 11's
numbered list): the LORE HEADER ruling change (10 Aug 2026 defect
ruling, branch `design/h-defects`, commit `0a46d3b`, not yet merged
into this branch's code) is now recorded in
`docs/CRESTFALL-DESIGN-CONTEXT.md` (two spots: the Lore ruling
paragraph and the Sprint G wave-3 rollup line) and
`docs/BUILD-BLUEPRINT.md` (section 3.1's Lore row-9 narrative),
superseding the 9 Aug 2026 centered-editorial-label ruling. A new row
was also added to the `/studio/v2/lore` CSV rows (item 5 above) noting
the current base branch still renders centered markup pending that
branch's merge.

Not executed, flagged for Brian rather than guessed: `docs/APP-FUNCTION-MAP.md`
states at its own top that it is "Generated from
docs/APP-FUNCTION-MAP.csv by a script. Do not edit by hand." No such
script exists anywhere in this repo (searched for a generator under
any name; none found). This session did not hand-write 1000+ lines to
approximate a script it cannot verify, so the rollup's Coverage line
and Flags list are now stale relative to the CSV changes in this
report (34 new controls added for Adventures, Lore, and `/lore`; 2 new
controls added for Community's filter bar). Candidate for a queue
item: write the generator, or rule that the rollup is regenerated by
hand at wave boundaries.

Verification for this pass: `docs/APP-FUNCTION-MAP.csv` parses clean
(1045 rows including header, 21 columns every row, checked
programmatically); zero em dashes in every file this pass touched;
production build exit 0 (section 14). No component file edited. The
other session's folder and the crestfall-main sibling checkout were
not touched.

## 14. H6 catch-up verification

- Production build: `npm run build` exit 0 on `design/h6-rows`, all
  three `/studio/v2/adventures`, `/studio/v2/lore`, and `/studio/v2/community`
  routes compile as dynamic routes alongside the rest of the set.
  Docs-only branch; no component file changed, so this confirms
  nothing broke, not new rendering.
- Em dashes: zero across every file this pass touched
  (`docs/APP-FUNCTION-MAP.csv`, `docs/PARITY-AUDIT.md`,
  `docs/CRESTFALL-DESIGN-CONTEXT.md`, `docs/BUILD-BLUEPRINT.md`).
- Shared docs touched this pass, exclusively: `docs/APP-FUNCTION-MAP.csv`,
  `docs/CRESTFALL-DESIGN-CONTEXT.md`, `docs/BUILD-BLUEPRINT.md`, and
  this file. `docs/APP-FUNCTION-MAP.md` (the CSV rollup),
  `docs/DESIGN-TOKENS.md`, and `docs/CONTRACT-REQUESTS.md` were left
