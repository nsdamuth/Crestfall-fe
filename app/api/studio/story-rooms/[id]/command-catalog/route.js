...
function apiError(
  message,
  status = 500,
  code = "STORY_ROOM_COMMAND_CATALOG_FAILED"
) {
  ...
  try {
    const responsePayload = await crestfallApiRequest({
      path: `/v1/studio/story-rooms/${encodeURIComponent(
        roomId
      )}/command-catalog?requestedSpeakerId=${encodeURIComponent(
        requestedSpeakerId
      )}`,
      method: "GET",
    ...
    return apiError(
      error.message || "Story room commands could not be loaded.",
      500,
      "STORY_ROOM_COMMAND_CATALOG_PROXY_FAILED"
    );
  }
}
