...
function apiError(message, status = 500, code = "ABILITY_SPELL_CHARACTER_CONFIGURATION_FAILED") {
...
export async function GET(_request, { params }) {
  ...
  try {
    const responsePayload = await crestfallApiRequest({
      path: `/v1/studio/story-rooms/${encodeURIComponent(roomId)}/character-configuration/ability-spell`,
      method: "GET",
    ...
    return apiError(
      requestError.message || "Ability/Spell Character Configuration could not be loaded.",
      Number.isInteger(requestError?.status) ? requestError.status : 500,
      requestError?.code || "ABILITY_SPELL_CHARACTER_CONFIGURATION_LOAD_FAILED"
    );
  }
}
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    return apiError(
      requestError.message || "Ability/Spell Character Configuration could not be saved.",
      Number.isInteger(requestError?.status) ? requestError.status : 500,
      requestError?.code || "ABILITY_SPELL_CHARACTER_CONFIGURATION_SAVE_FAILED"
    );
  }
}
