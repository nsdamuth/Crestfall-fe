...
function apiError(message, status = 500, code = "STATS_POOLS_CHARACTER_CONFIGURATION_FAILED") {
...
export async function GET(_request, { params }) {
  ...
  try {
    const responsePayload = await crestfallApiRequest({
      path: `/v1/studio/story-rooms/${encodeURIComponent(roomId)}/character-configuration/stats`,
      method: "GET",
    ...
    return apiError(
      requestError.message || "Stats & Pools Character Configuration could not be loaded.",
      Number.isInteger(requestError?.status) ? requestError.status : 500,
      requestError?.code || "STATS_POOLS_CHARACTER_CONFIGURATION_LOAD_FAILED"
    );
  }
}
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    return apiError(
      requestError.message || "Stats & Pools Character Configuration could not be saved.",
      Number.isInteger(requestError?.status) ? requestError.status : 500,
      requestError?.code || "STATS_POOLS_CHARACTER_CONFIGURATION_SAVE_FAILED"
    );
  }
}
