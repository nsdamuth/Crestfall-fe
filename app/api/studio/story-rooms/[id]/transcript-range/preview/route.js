...
function apiError(message, status = 500, code = "TRANSCRIPT_RANGE_PREVIEW_FAILED") {
...
export async function POST(request, { params }) {
  ...
  try {
    const responsePayload = await crestfallApiRequest({
      path: `/v1/studio/story-rooms/${encodeURIComponent(roomId)}/transcript-range/preview`,
      method: "POST",
      body: payload,
    ...
    return apiError(
      error.message || "Transcript range could not be previewed.",
      500
    );
  }
}
