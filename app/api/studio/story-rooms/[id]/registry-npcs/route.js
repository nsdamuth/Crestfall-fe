...
export async function GET(_request, { params }) {
  ...
  try {
    const responsePayload = await crestfallApiRequest({
      path: `/v1/studio/story-rooms/${encodeURIComponent(roomId)}/registry-npcs`,
      method: "GET",
      headers: {
    ...
    return apiError(
      error.message || "NPC Registry participants could not be loaded.",
      500,
      "REGISTRY_NPC_LIST_FAILED"
    );
  }
}
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    return apiError(
      error.message || "NPC Registry participant could not be loaded.",
      500,
      "REGISTRY_NPC_LOAD_FAILED"
    );
  }
}
