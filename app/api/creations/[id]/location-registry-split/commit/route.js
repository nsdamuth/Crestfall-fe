...
export async function POST(request, { params }) {
  ...
  try {
    const responsePayload = await crestfallApiRequest({
      path: `/v1/studio/creations/${encodeURIComponent(creationId)}/location-registry-split/commit`,
      method: "POST",
      body,
    ...
    return forwardServiceError(
      error,
      "Location Registry split could not be applied.",
      "LOCATION_REGISTRY_SPLIT_COMMIT_FAILED"
    );
  }
}
