...
import { cancelOwnedLoreValidation } from "@/lib/server/services/creations/loreValidationService";

export const dynamic = "force-dynamic";
export const revalidate = 0;

function apiError(message, status = 500, code = "LORE_VALIDATION_CANCEL_FAILED") {
...
export async function POST(_request, { params }) {
  ...
  if (!creationId || !submissionId) {
    return apiError(
      ...
      400,
      "LORE_VALIDATION_CANCEL_IDS_REQUIRED"
    );
  }

  try {
    const payload = await cancelOwnedLoreValidation({
      userId: user.id,
      creationId,
      submissionId,
    });
    ...
    return apiError(
      ...
      500,
      error?.code || "LORE_VALIDATION_CANCEL_FAILED"
    );
  }
}
