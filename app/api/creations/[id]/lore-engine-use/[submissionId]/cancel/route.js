...
import { cancelOwnedLoreEngineUseSubmission } from "@/lib/server/services/creations/loreEngineUseService";

export const dynamic = "force-dynamic";
export const revalidate = 0;

function apiError(message, status = 500, code = "LORE_ENGINE_USE_CANCEL_FAILED") {
...
export async function POST(_request, { params }) {
  ...
  if (!creationId || !submissionId) {
    return apiError(
      ...
      400,
      "LORE_ENGINE_USE_CANCEL_IDS_REQUIRED"
    );
  }

  try {
    const payload = await cancelOwnedLoreEngineUseSubmission({
      userId: user.id,
      creationId,
      submissionId,
    });
    ...
    return apiError(
      error?.message || "Lore engine-use submission could not be cancelled.",
      500,
      error?.code || "LORE_ENGINE_USE_CANCEL_FAILED"
    );
  }
}
