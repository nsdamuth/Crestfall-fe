...
import EyeColorModalView from "@/components/studio/create/character/eye-color/EyeColorModal.view";
import {
  eyeColorClosedFixture,
  eyeColorCustomFixture,
  eyeColorEmptyFixture,
  eyeColorLongContentFixture,
  eyeColorPresetFixture,
} from "@/components/studio/create/character/eye-color/EyeColorModal.fixtures";

const PREVIEW_STATES = {
  closed: {
    label: "Closed Trigger",
    props: eyeColorClosedFixture,
  },
  preset: {
    label: "Preset Value",
    props: eyeColorPresetFixture,
  },
  empty: {
    label: "Not Chosen",
    props: eyeColorEmptyFixture,
  },
  custom: {
    label: "Custom Value",
    props: eyeColorCustomFixture,
  },
  longContent: {
    label: "Long Content",
    props: eyeColorLongContentFixture,
  },
};
...
export default function EyeColorPreviewClient() {
  const [activeStateKey, setActiveStateKey] = useState("closed");
  const [previewProps, setPreviewProps] = useState(() =>
    cloneFixture(eyeColorClosedFixture)
  );
  ...
  function chooseOption(optionId) {
    ...
    setLastAction(
      `Eye color changed to ${getOptionLabel(
    ...
    <main className="min-h-screen bg-[#080706] px-4 py-10 text-[var(--foreground)] sm:px-6">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="rounded-2xl border border-[var(--muted-gold)]/25 bg-black/35 p-6">
          ...
          <h1 className="mt-2 font-display text-4xl">Eye Color Modal</h1>
        ...
        <section className="rounded-2xl border border-white/10 bg-black/25 p-5">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
            The View receives display-ready eye-color swatches and semantic
