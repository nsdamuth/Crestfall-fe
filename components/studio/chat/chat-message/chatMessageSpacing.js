import { CHAT_MESSAGE_SEGMENT_TYPES } from "./ChatMessage.contract";
...
function getSemanticPartKind(segment) {
  if (segment?.type === CHAT_MESSAGE_SEGMENT_TYPES.DIALOGUE) {
    return MESSAGE_PART_KINDS.DIALOGUE;
  }

  if (segment?.type === CHAT_MESSAGE_SEGMENT_TYPES.NARRATION) {
...
function groupAlternatingMessageParts(parts, getKind) {
  ...
  parts.forEach((part) => {
    ...
    if (kind !== MESSAGE_PART_KINDS.NEUTRAL && meaningfulKinds.size === 2) {
...
function tokenizeLegacyMessageParts(body) {
  ...
  if (lastIndex < text.length) {
    mergeLegacyPart(parts, text.slice(lastIndex), MESSAGE_PART_KINDS.NARRATION);
  }
...
export function buildLegacyMessageParagraphs(body = "") {
  ...
  const groupedParts = groupAlternatingMessageParts(parts, (part) => part.kind);
