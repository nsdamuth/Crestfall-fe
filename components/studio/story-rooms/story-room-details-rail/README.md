...
- the authoritative latest-responder image as a dedicated primary surface from `room.featuredSpeakerImageUrl` / `room.featuredSpeakerName`, plus a separate secondary media set interim until CR-069 serves a list: cast media/avatars and scene images, deduplicated by url and excluding the primary responder image; when the snapshot serves none, the catalogue creation's own featured media from its preview (`fetchCreationPreview`, the same served images the community story slider shows); at most four secondary images
