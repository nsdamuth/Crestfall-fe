...
const SHARE_LINK_STATE_IDS = new Set(Object.values(CHAT_SHARE_LINK_STATES));
...
function normalizeShareStatus(value) {
  const normalized = text(value).toUpperCase();
  return SHARE_LINK_STATE_IDS.has(normalized) ? normalized : "FAILED";
}
...
export function buildStorySessionVisibleMessageOptions(messages = []) {
  return array(messages)
    ...
    .map((message, index) => {
      const body = text(message.body).replace(/\s+/g, " ").trim();
...
export function getStorySessionDefaultRangeBoundaries(messageOptions = []) {
...
export function validateStorySessionRangeSelection({
  ...
  if (customRange && (!text(startMessageId) || !text(endMessageId))) {
    return {
      valid: false,
      preset: normalizedPreset,
      customRange,
      error: "Choose both a start message and an end message.",
    };
  }
...
export function buildStorySessionExportIntent({
  ...
  if (!range.valid) {
    return { valid: false, error: range.error, payload: null };
  }
  ...
  return {
    ...
    payload: {
      preset: range.preset,
      startMessageId: range.customRange ? text(startMessageId) : null,
      endMessageId: range.customRange ? text(endMessageId) : null,
...
export function projectStorySessionShareResult({
  ...
  const shareMode = normalizeShareMode(source.shareMode || requestedMode);
  ...
  return {
    ...
    shareMode,
    expiresAt:
      active && shareMode === "TEMPORARY" ? text(source.expiresAt) : "",
  };
}
...
export function projectStorySessionExportDialog({
  ...
  const messageOptions = buildStorySessionVisibleMessageOptions(messages);
  const normalizedPreset = normalizePreset(preset);
  ...
  return {
    ...
    open: Boolean(open),
    presets: CHAT_EXPORT_RANGE_PRESETS.map((presetOption) => ({ ...presetOption })),
    preset: normalizedPreset,
    formats: CHAT_EXPORT_FORMAT_OPTIONS.map((formatOption) => ({ ...formatOption })),
    format: normalizeFormat(format),
    ...
    error: text(error),
    onPresetChange: callbackSource.onExportPresetChange || null,
    onFormatChange: callbackSource.onExportFormatChange || null,
    onStartMessageChange: callbackSource.onExportStartMessageChange || null,
    onEndMessageChange: callbackSource.onExportEndMessageChange || null,
    onSubmit: callbackSource.onExportSubmit || null,
    onClose: callbackSource.onExportClose || null,
  };
}
...
export function projectStorySessionShareDialog({
  ...
  return {
    ...
    onModeChange: callbackSource.onShareModeChange || null,
    onPresetChange: callbackSource.onSharePresetChange || null,
    onStartMessageChange: callbackSource.onShareStartMessageChange || null,
    onEndMessageChange: callbackSource.onShareEndMessageChange || null,
    onSubmit: callbackSource.onShareSubmit || null,
    onCopy: callbackSource.onShareCopy || null,
    onRequestRevoke: callbackSource.onShareRequestRevoke || null,
    onConfirmRevoke: callbackSource.onShareConfirmRevoke || null,
    onCancelRevoke: callbackSource.onShareCancelRevoke || null,
    onClose: callbackSource.onShareClose || null,
  };
}
...
export function projectStorySessionShareExportActions({
  ...
  return {
    ...
    messageOptions,
    defaultRange: getStorySessionDefaultRangeBoundaries(messageOptions),
  };
}
