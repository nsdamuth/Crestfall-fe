...
export default function MechanicsActionAvailabilitySection({ form, updateDataField, ownerLabel = "this Story" }) {
  ...
  return <div><SectionTitle title="Available Actions" body={`Make reusable Mechanics Actions available to ${ownerLabel}. Action Sets are the normal path for shared rules; direct Actions are available for small exceptions.`}/>
    <div className="mt-6 grid gap-5">
      ...
      <div className="rounded-[var(--radius-md)] border border-white/10 bg-[var(--surface-1)] p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex gap-3"><div className="rounded-xl border border-white/10 bg-black/20 p-3 text-[var(--gold-ornament)]"><Activity size={20}/></div><div><p className="text-xs uppercase tracking-[0.22em] text-[var(--gold-ornament)]">Direct Actions</p><h3 className="mt-2 font-display text-2xl">Advanced exceptions</h3><p className="mt-2 text-sm text-[var(--ink-dim)]">Attach individual Actions only when a reusable Set would be unnecessary.</p></div></div><button type="button" onClick={() => setPickerType("MECHANICS_ACTION")} disabled={actionRefs.length >= MAX_DIRECT} className="cf-btn cf-btn--secondary"><Plus size={14}/> Attach Action</button></div>
