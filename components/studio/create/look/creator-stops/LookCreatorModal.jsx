...
// quick creates use (contract creator-stops.view.v6).
...
import PaletteStopView from "./palette-stop/PaletteStopView";
...
// content_rating, data) the Character and World quick creates post to
// /v1/studio/creations. type "OUTFIT": the existing creation type this
// app's data model already uses for outfit/clothing assets
// (lib/shared/creations/creationTypePolicy.js), per the brief's
// instruction to map onto the existing outfit door and type rather
// than inventing a new one.
function buildSaveCreationPayload(formState) {
  const name = formState.name?.trim() || "Unnamed Look";

  return {
    type: "OUTFIT",
    title: name,
    description:
      formState.vibe || formState.garments || "A private draft look created in Crestfall Studio.",
    visibility: "PRIVATE",
    content_rating: "SFW",
    data: {
      ...formState,
...
export default function LookCreatorModal({ onClose }) {
  const router = useRouter();
  const [activeStop, setActiveStop] = useState(LOOK_STOP_IDS[0]);
  ...
  function handleContinueInEditorAfterSave() {
    ...
    onClose?.();
    // Origin tracking, same convention as the Character and World
    // quick creates: carries the opening surface so the advanced
