...
// Local to the World quick create, mirroring the Character quick
// create's own package-local shared/Controls.jsx (docs/STUDIO-SPEC.md
// section 3.2 precedent): the field list differs, so this package
// carries its own copy of the handful of primitives it needs rather
// than importing Character's, keeping the two packages independently
// editable. Same recipes, same classes, same law.

// Standard gold eyebrow recipe, RULED 11 Aug 2026 (Sprint H render
// review, item 2): the same component the nine v2 page headers and
// the Character quick create use.
  ...
  return (
    <div>
      ...
      <input
        ...
        className="min-h-[var(--control-md)] w-full rounded-[var(--radius-md)] border border-[var(--line-whisper)] bg-[var(--bed-deep)] shadow-[var(--shadow-bed)] px-[var(--space-4)] py-[var(--space-2)] text-[var(--text-body)] leading-[var(--lh-body)] text-[var(--ink)] outline-none transition placeholder:text-[var(--ink-dim)]"
      />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <textarea
        ...
        placeholder={placeholder}
        rows={1}
        className="w-full resize-none overflow-y-auto rounded-[var(--radius-md)] border border-[var(--line-whisper)] bg-[var(--bed-deep)] shadow-[var(--shadow-bed)] px-[var(--space-4)] py-[var(--space-2)] text-sm leading-6 text-[var(--ink)] outline-none transition placeholder:text-[var(--ink-dim)]"
        style={{
