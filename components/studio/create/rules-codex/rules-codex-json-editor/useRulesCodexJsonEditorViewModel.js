...
} from "./rulesCodexJsonEditor.validation";
import {
  RULES_CODEX_JSON_AI_AUTHORING_GUIDE_FILENAME,
  RULES_CODEX_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
  buildRulesCodexJsonAiAuthoringGuide,
} from "./rulesCodexJsonAiAuthoringGuide";

export function useRulesCodexJsonEditorViewModel({
  rulesCodex = {},
  onApply = null,
  onClose = null,
} = {}) {
  const sourceText = useMemo(
    () => formatRulesCodexJsonData(rulesCodex),
    [rulesCodex]
  );
  ...
  async function handleCopy() {
    try {
      ...
      setStatusMessage("Rules Codex JSON copied to the clipboard.");
    } catch (error) {
      setCopyStatus("error");
      setStatusMessage(error?.message || "Rules Codex JSON could not be copied.");
    }
  }
  ...
  function handleDownloadAiGuide() {
    ...
    try {
      ...
      const guideText = buildRulesCodexJsonAiAuthoringGuide(rulesCodex);
      const guideBlob = new Blob([guideText], {
        type: RULES_CODEX_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
      });
      objectUrl = URL.createObjectURL(guideBlob);
      const anchor = document.createElement("a");
      anchor.href = objectUrl;
      anchor.download = RULES_CODEX_JSON_AI_AUTHORING_GUIDE_FILENAME;
      anchor.rel = "noopener";
      ...
      setStatusMessage(
        "AI authoring guide downloaded with the current Rules Codex JSON, activation contracts, and interpretation-only authority boundaries."
      );
    } catch (error) {
  ...
  function handleFormat() {
    const result = formatRulesCodexJsonText(jsonText);
    if (!result.valid) {
    ...
    setErrors([]);
    setWarnings([]);
    setStatusMessage("JSON formatted. No Rules Codex builder data was changed.");
  }
  ...
  function handleValidateAndApply() {
    const result = validateRulesCodexJsonText(jsonText);
    ...
    setStatusMessage(
      result.warnings.length
        ...
        : "Rules Codex JSON validated and applied."
    );
    onClose?.();
  }

  return {
    title: "Rules Codex JSON Editor",
    description:
      "Copy, inspect, or replace the complete authored Rules Codex. Validate & Apply updates the current visual editor only; use the page Save action to persist it.",
    jsonText,
    errors,
    warnings,
    statusMessage,
