...
} from "./skillsProfileJsonEditor.validation";
import {
  SKILLS_PROFILE_JSON_AI_AUTHORING_GUIDE_FILENAME,
  SKILLS_PROFILE_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
  buildSkillsProfileJsonAiAuthoringGuide,
} from "./skillsProfileJsonAiAuthoringGuide";

export function useSkillsProfileJsonEditorViewModel({
  value = {},
  onApply = null,
  onClose = null,
} = {}) {
  const sourceText = useMemo(() => formatSkillsProfileJsonData(value), [value]);
  ...
  async function handleCopy() {
    try {
      ...
      setStatusMessage("Skills Profile JSON copied to the clipboard.");
    } catch (error) {
      setCopyStatus("error");
      setStatusMessage(
        error?.message || "Skills Profile JSON could not be copied."
      );
    }
  }
  ...
  function handleDownloadAiGuide() {
    ...
    try {
      ...
      const guideText = buildSkillsProfileJsonAiAuthoringGuide(value);
      const guideBlob = new Blob([guideText], {
        type: SKILLS_PROFILE_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
      });
      objectUrl = URL.createObjectURL(guideBlob);
      const anchor = document.createElement("a");
      anchor.href = objectUrl;
      anchor.download = SKILLS_PROFILE_JSON_AI_AUTHORING_GUIDE_FILENAME;
      anchor.rel = "noopener";
      ...
      setStatusMessage(
        "AI authoring guide downloaded with the current Skills Profile JSON and definition-only instructions."
      );
    } catch (error) {
  ...
  function handleFormat() {
    const result = formatSkillsProfileJsonText(jsonText);
    if (!result.valid) {
    ...
    setErrors([]);
    setWarnings([]);
    setStatusMessage("JSON formatted. No Skills Profile builder data was changed.");
  }
  ...
  function handleValidateAndApply() {
    const result = validateSkillsProfileJsonText(jsonText);
    ...
    setStatusMessage(
      result.warnings.length
        ...
        : "Skills Profile JSON validated and applied."
    );
    onClose?.();
  }

  return {
    title: "Skills Profile JSON Editor",
    description:
      "Copy, inspect, or replace the complete authored Skills Profile. Validate & Apply updates the current visual editor only; use the page Save action to persist it.",
    jsonText,
    errors,
    warnings,
    statusMessage,
