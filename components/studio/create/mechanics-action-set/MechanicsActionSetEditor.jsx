  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-2">
        <label className="block"><span className="text-xs uppercase tracking-[0.18em] text-[var(--gold-ornament)]">Stable Action Set key</span><input value={normalizeString(source.id)} readOnly={keyLocked} onChange={(event) => patch("id", event.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-black/35 px-4 py-3 text-sm read-only:opacity-60" /></label>
        <label className="block"><span className="text-xs uppercase tracking-[0.18em] text-[var(--gold-ornament)]">Set title</span><input value={normalizeString(source.title)} onChange={(event) => patch("title", event.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-black/35 px-4 py-3 text-sm" /></label>
      </div>
      <label className="block"><span className="text-xs uppercase tracking-[0.18em] text-[var(--gold-ornament)]">Description</span><textarea value={normalizeString(source.description)} onChange={(event) => patch("description", event.target.value)} className="mt-2 min-h-24 w-full resize-y rounded-xl border border-white/10 bg-black/35 px-4 py-3 text-sm" /></label>
