  ...
  return (
    <section className="mt-8 grid gap-6 xl:grid-cols-[0.34fr_1fr]">
      {/* MOBILE-SHELLS: min-w-0 on both grid children. Without it the
          implicit base column is sized by min-width:auto, so the collapsed
          textarea preview spans inside this page (whitespace-nowrap through
          truncate) set the page's horizontal scroll width. This is the
          single-column rule for this shell: one column in DOM order below
          xl, each child free to shrink to the gutter. */}
      <aside className="min-w-0 self-start rounded-[var(--radius-md)] border border-[var(--gold-ornament)]/20 bg-[var(--surface-1)] p-5 xl:sticky xl:top-24">
        ...
        <div className="mt-5 grid gap-3 text-sm md:grid-cols-2 md:divide-x sm:divide-[var(--line-whisper)]">
      ...
      <div className="min-w-0 space-y-6">
        <section className="min-w-0 rounded-[var(--radius-md)] border border-[var(--gold-ornament)]/20 bg-[var(--surface-1)] p-5 sm:p-6">
          <p className="flex items-center gap-[var(--space-3)] text-[length:var(--text-eyebrow)] leading-[var(--lh-eyebrow)] font-medium uppercase tracking-[var(--track-eyebrow)] text-[var(--gold-ornament)]">
            Asset Identity
          </p>
