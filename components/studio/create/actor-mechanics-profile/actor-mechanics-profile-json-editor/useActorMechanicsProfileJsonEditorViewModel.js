...
} from "./actorMechanicsProfileJsonEditor.validation";
import {
  ACTOR_MECHANICS_PROFILE_JSON_AI_AUTHORING_GUIDE_FILENAME,
  ACTOR_MECHANICS_PROFILE_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
  buildActorMechanicsProfileJsonAiAuthoringGuide,
} from "./actorMechanicsProfileJsonAiAuthoringGuide";

export function useActorMechanicsProfileJsonEditorViewModel({
  ...
  const sourceText = useMemo(
    () => formatActorMechanicsProfileJsonData(actorMechanicsProfile, ownerContext),
    [actorMechanicsProfile, ownerContext]
  );
  ...
  async function handleCopy() {
    try {
      ...
      setStatusMessage("Actor Mechanics Profile JSON copied to the clipboard.");
    } catch (error) {
      setCopyStatus("error");
      setStatusMessage(
        error?.message || "Actor Mechanics Profile JSON could not be copied."
      );
    }
  }
  ...
  function handleDownloadAiGuide() {
    ...
    try {
      ...
      const guideText = buildActorMechanicsProfileJsonAiAuthoringGuide(actorMechanicsProfile);
      const guideBlob = new Blob([guideText], {
        type: ACTOR_MECHANICS_PROFILE_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
      });
      objectUrl = URL.createObjectURL(guideBlob);
      const anchor = document.createElement("a");
      anchor.href = objectUrl;
      anchor.download = ACTOR_MECHANICS_PROFILE_JSON_AI_AUTHORING_GUIDE_FILENAME;
      anchor.rel = "noopener";
      ...
      setStatusMessage(
        "AI authoring guide downloaded with the current Actor Mechanics Profile JSON and binding and actor-state boundary instructions."
      );
    } catch (error) {
  ...
  function handleFormat() {
    const result = formatActorMechanicsProfileJsonText(jsonText);
    if (!result.valid) {
    ...
    setWarnings([]);
    setStatusMessage(
      "JSON formatted. No Actor Mechanics Profile builder data was changed."
    );
  }
  ...
  function handleValidateAndApply() {
    const result = validateActorMechanicsProfileJsonText(jsonText, ownerContext);
    ...
    setStatusMessage(
      result.warnings.length
        ...
        : "Actor Mechanics Profile JSON validated and applied."
    );
    onClose?.();
  }

  return {
    title: "Actor Mechanics Profile JSON Editor",
    description:
      "Copy, inspect, or replace the complete authored Actor Mechanics Profile. Validate & Apply updates the current visual editor only; use the page Save action to persist it.",
    jsonText,
    errors,
    warnings,
    statusMessage,
