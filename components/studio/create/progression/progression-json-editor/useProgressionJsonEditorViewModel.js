...
} from "./progressionJsonEditor.validation";
import {
  PROGRESSION_JSON_AI_AUTHORING_GUIDE_FILENAME,
  PROGRESSION_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
  buildProgressionJsonAiAuthoringGuide,
} from "./progressionJsonAiAuthoringGuide";

export function useProgressionJsonEditorViewModel({
  progressionProfile = {},
  onApply = null,
  onClose = null,
} = {}) {
  const sourceText = useMemo(
    () => formatProgressionJsonData(progressionProfile),
    [progressionProfile]
  );
  ...
  async function handleCopy() {
    try {
      ...
      setStatusMessage("Progression Profile JSON copied to the clipboard.");
    } catch (error) {
      setCopyStatus("error");
      setStatusMessage(
        error?.message || "Progression Profile JSON could not be copied."
      );
    }
  }
  ...
  function handleDownloadAiGuide() {
    ...
    try {
      ...
      const guideText = buildProgressionJsonAiAuthoringGuide(
        progressionProfile
      );
      const guideBlob = new Blob([guideText], {
        type: PROGRESSION_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
      });
      ...
      anchor.download = PROGRESSION_JSON_AI_AUTHORING_GUIDE_FILENAME;
      anchor.rel = "noopener";
      ...
      setStatusMessage(
        "AI authoring guide downloaded with the current Progression Profile JSON and definition-only instructions."
      );
    } catch (error) {
  ...
  function handleFormat() {
    const result = formatProgressionJsonText(jsonText);
    ...
    setWarnings([]);
    setStatusMessage(
      "JSON formatted. No Progression Profile builder data was changed."
    );
  }
  ...
  function handleValidateAndApply() {
    const result = validateProgressionJsonText(jsonText);
    ...
    setStatusMessage(
      result.warnings.length
        ...
        : "Progression Profile JSON validated and applied."
    );
    onClose?.();
  }
  ...
  return {
    ...
    description:
      "Copy, inspect, or replace the complete authored Progression Profile. Validate & Apply updates the current visual editor only; use the page Save action to persist it.",
    jsonText,
    errors,
    warnings,
    statusMessage,
