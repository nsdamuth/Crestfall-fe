...
} from "./statsPoolsJsonEditor.validation";
import {
  STATS_POOLS_JSON_AI_AUTHORING_GUIDE_FILENAME,
  STATS_POOLS_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
  buildStatsPoolsJsonAiAuthoringGuide,
} from "./statsPoolsJsonAiAuthoringGuide";

export function useStatsPoolsJsonEditorViewModel({
  statsPoolsProfile = {},
  onApply = null,
  onClose = null,
} = {}) {
  const sourceText = useMemo(
    () => formatStatsPoolsJsonData(statsPoolsProfile),
    [statsPoolsProfile]
  );
  ...
  async function handleCopy() {
    try {
      ...
      setStatusMessage("Stats & Pools Profile JSON copied to the clipboard.");
    } catch (error) {
      setCopyStatus("error");
      setStatusMessage(
        error?.message || "Stats & Pools Profile JSON could not be copied."
      );
    }
  }
  ...
  function handleDownloadAiGuide() {
    ...
    try {
      ...
      const guideText = buildStatsPoolsJsonAiAuthoringGuide(statsPoolsProfile);
      const guideBlob = new Blob([guideText], {
        type: STATS_POOLS_JSON_AI_AUTHORING_GUIDE_MIME_TYPE,
      });
      objectUrl = URL.createObjectURL(guideBlob);
      const anchor = document.createElement("a");
      anchor.href = objectUrl;
      anchor.download = STATS_POOLS_JSON_AI_AUTHORING_GUIDE_FILENAME;
      anchor.rel = "noopener";
      ...
      setStatusMessage(
        "AI authoring guide downloaded with the current Stats & Pools Profile JSON and definition-only instructions."
      );
    } catch (error) {
  ...
  function handleFormat() {
    const result = formatStatsPoolsJsonText(jsonText);
    if (!result.valid) {
    ...
    setWarnings([]);
    setStatusMessage(
      "JSON formatted. No Stats & Pools Profile builder data was changed."
    );
  }
  ...
  function handleValidateAndApply() {
    const result = validateStatsPoolsJsonText(jsonText);
    ...
    setStatusMessage(
      result.warnings.length
        ...
        : "Stats & Pools Profile JSON validated and applied."
    );
    onClose?.();
  }
  ...
  return {
    ...
    description:
      "Copy, inspect, or replace the complete authored Stats & Pools Profile. Validate & Apply updates the current visual editor only; use the page Save action to persist it.",
    jsonText,
    errors,
    warnings,
    statusMessage,
