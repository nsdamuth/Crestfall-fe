...
export const STORY_OPENING_LOCATION_MODE_OPTIONS = Object.freeze([
  { value: STORY_OPENING_LOCATION_MODES.FIXED, label: "Fixed Starting Location" },
  {
...
export function normalizeStoryOpeningLocationReference(reference) {
  ...
  return {
    ...
    imageUrl:
      normalizeString(
        source.imageUrl || source.image_url || source.thumbnailUrl || source.url
      ) || null,
  };
}
...
function uniqueReferences(references) {
  ...
  normalizeArray(references).forEach((reference) => {
    const normalized = normalizeStoryOpeningLocationReference(reference);
...
export function normalizeStoryOpeningLocationAuthoring(
  ...
  const mode = Object.values(STORY_OPENING_LOCATION_MODES).includes(rawMode)
    ? rawMode
  ...
  const storedById = new Map(storedAllowed.map((reference) => [reference.id, reference]));
  ...
  const legacyFixedLocationId = normalizeString(
    story.location_id || story.locationId || story.selected_location?.id
  );
...
export function buildPlayerSelectableOpeningLocationConfig(locations = []) {
...
export function buildFixedOpeningLocationConfig(location = null) {
  const fixedLocation = normalizeStoryOpeningLocationReference(location);
  return {
    version: STORY_OPENING_LOCATION_AUTHORING_VERSION,
...
export function toggleOpeningLocationReference(locations = [], location = null) {
  const normalizedLocation = normalizeStoryOpeningLocationReference(location);
  const current = uniqueReferences(locations);
  if (!normalizedLocation?.id) return current;

  return current.some((item) => item.id === normalizedLocation.id)
    ? current.filter((item) => item.id !== normalizedLocation.id)
